...
def get_acc(tgt_df):
    return 100 * tgt_df['correct'].sum() / len(tgt_df)
...
def parse_params_from_filename(pred_model_name):
...
def evaluate(pred_filepath, gt_filepath):
    ...
    ## Read data
    pred_list = read_json(pred_filepath)
    gt_list = read_json(gt_filepath)
    ...
    gt_df = pd.DataFrame(gt_list)[['question_id', 'videoID', 'duration', 'domain', 'task_type', 'sub_category']]
...
if __name__ == "__main__":
    ...
    # pred_filepath = "output/llava-video-7b-qwen2-video-only_F-128_fps-1.json"
    # pred_filepath = "output/llava-video-7b-qwen2_F-128_fps-1.json"
