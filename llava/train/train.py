...
class ModelArguments:
    ...
    mm_resampler_model_name: Optional[str] = field(default=None)
    mm_resampler_skip_layer_flag: Optional[bool] = field(default=False)
    mm_resampler_post_flag: Optional[bool] = field(default=False)
    mm_resampler_grid_width: Optional[int] = field(default=0) # "-1: using all input tokens; X: indexing tokens per X interval - e.g., 15")
    mm_resampler_skip_last_norm: Optional[bool] = field(default=False)
    ...
    max_batch_size: Optional[int] = field(default=256)
    model_base: Optional[str] = field(default=None)

    slow_mm_spatial_pool_stride: Optional[int] = field(default=2)
    slow_path_stride: Optional[int] = field(default=10)
    slow_fast_path_flag: Optional[bool] = field(default=False)

    attn_implementation: str = field(default="flash_attention_2", metadata={"help": "Use transformers attention implementation."})
    ## Sparse attention common config
    sa_pattern: Optional[str] = field(default="")
    sa_start_layer_idx: Optional[int] = field(default=0) # 0 1 ...
    ...
    sa_tree_thresh: Optional[float] = field(default=-1.0)
    ...
    ## FrameFusion
    sa_framefusion_cost: Optional[float] = field(default=0.30)
...
class DataArguments:
    ...
    tgt_video_fps: Optional[int] = field(default=None)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    # only has mm_mlp_adapter and mm_vision_resampler in the tuneable parts
        ...
        mm_tunable_parts = trainer.args.mm_tunable_parts
        if (len(mm_tunable_parts.split(",")) == 1 and ("mm_mlp_adapter" in mm_tunable_parts or "mm_vision_resampler" in mm_tunable_parts)):
            check_only_save_mm_adapter_tunnable = True
        elif (len(mm_tunable_parts.split(",")) == 2 and ("mm_mlp_adapter" in mm_tunable_parts and "mm_vision_resampler" in mm_tunable_parts)):
            check_only_save_mm_adapter_tunnable = True
    else:
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    # You can convert back to tokens by "tokenizer.convert_ids_to_tokens(tgt_id_list)"
    return dict(
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if  model_args.mm_spatial_pool_out_channels is not None and model_args.mm_spatial_pool_mode is not None:
    ...
    if model_args.mm_resampler_type is not None:
        ...
        overwrite_config["mm_resampler_model_name"] = model_args.mm_resampler_model_name
        overwrite_config["mm_resampler_skip_layer_flag"] = model_args.mm_resampler_skip_layer_flag
        overwrite_config["mm_resampler_post_flag"] = model_args.mm_resampler_post_flag
        overwrite_config["mm_resampler_grid_width"] = model_args.mm_resampler_grid_width
        overwrite_config["mm_resampler_skip_last_norm"] = model_args.mm_resampler_skip_last_norm
    
    if model_args.mm_resampler_type == "qformer":
        overwrite_config["mm_qformer_depth"] = model_args.mm_qformer_depth
        overwrite_config["mm_qformer_latents"]  = model_args.mm_qformer_latents
        overwrite_config["mm_qformer_pretrained"]  = model_args.mm_qformer_pretrained
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.max_batch_size = model_args.max_batch_size
        model.config.instruction_aware_resampling = model_args.instruction_aware_resampling
