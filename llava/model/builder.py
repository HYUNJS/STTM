    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=cfg_pretrained, **kwargs)
            elif "gemma" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
            ...
            mm_projector_weights = {k: v.to(model.dtype) for k, v in mm_projector_weights.items()}
            missing_keys, unexpected_keys = model.load_state_dict(mm_projector_weights, strict=False)
            print(f"Loading mm_projector.bin\nMissing: {missing_keys}\nUnexpected: {unexpected_keys}")
        else:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs) ## This is the function called for loading pretrained llava-video
