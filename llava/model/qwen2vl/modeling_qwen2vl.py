...
from transformers.generation.utils import (
    GenerateOutput, GenerateNonBeamOutput, GenerateDecoderOnlyOutput, GenerateEncoderDecoderOutput, 
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        prompt_stat: Optional[Dict[str, int]] = None,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw) # [THW, C]
                video_mask = (input_ids == self.config.video_token_id).unsqueeze(-1).expand_as(inputs_embeds) # [B, N, C]
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        ### add prompt stat to model attribute
        prompt_stat = model_kwargs["prompt_stat"]
        self.model.image_token_start_index = torch.tensor(prompt_stat["sys"])
        self.model.image_token_length = torch.tensor(prompt_stat["video"])
        self.model.num_frame = torch.tensor(prompt_stat["frame"])
        self.model.image_T = torch.tensor(prompt_stat["T"])
        self.model.image_H = torch.tensor(prompt_stat["H"])
        self.model.image_W = torch.tensor(prompt_stat["W"])
        self.model.patch_num = torch.tensor(prompt_stat["H"] * prompt_stat["W"])
        ###
        num_iter = 0
        ttft_llm, time_decoding = 0, 0
        while self._has_unfinished_sequences(
            ...
            end_time = time.time()
            if num_iter == 0:
                ttft_llm = end_time - start_time
                num_last_layer_token = outputs.past_key_values[-1][0].size(-2)
                model_kwargs['prompt_stat']['num_last_layer_token'] = num_last_layer_token
            else:
                time_decoding += end_time - start_time
            num_iter += 1
