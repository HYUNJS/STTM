...
def prepare_inputs_labels_for_multimodal_get_patch_type(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
    ...
    ### FRAMEFUSION START ###
    spatial_pool_stride_size = self.config.mm_spatial_pool_stride
    if self.config.mm_spatial_pool_mode == "bilinear":
        patch_size = math.ceil(self.get_vision_tower().num_patches_per_side / spatial_pool_stride_size)
    else:
        patch_size = self.get_vision_tower().num_patches_per_side // spatial_pool_stride_size
    ...
    if mm_newline_position == "grid":
        ...
        raise ValueError(f"Unexpected mm_newline_position for FrameFusion: {mm_newline_position}")
