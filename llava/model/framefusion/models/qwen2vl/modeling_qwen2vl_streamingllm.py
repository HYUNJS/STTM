...
try:
    from minference.ops.streaming_kernel import streaming_forward
except ImportError:
    # minference is not needed if streamingllm is not used
    streaming_forward = None


def replace_Qwen2VL_streamingllm(model, init_num = 4, length_rate = 0.3):
    ...
    if isinstance(model.model, Qwen2VLModel):
        model.model.forward = MethodType(Qwen2VLModel_streamingllm_forward, model.model)
        
    for i, decoder_layer in enumerate(model.model.layers):
        if isinstance(decoder_layer, Qwen2VLDecoderLayer):
            decoder_layer.forward = MethodType(Qwen2VLDecoderLayer_streamingllm_forward, decoder_layer)
            
        qwen2vl_attention_instance = decoder_layer.self_attn
        if isinstance(qwen2vl_attention_instance, Qwen2VLFlashAttention2):
            qwen2vl_attention_instance.forward = MethodType(partial(Qwen2VLFlashAttention2_streamingllm_forward, model=model), qwen2vl_attention_instance)
        else:
            raise TypeError("language model is not Qwen2VL.")
     
def Qwen2VLFlashAttention2_streamingllm_forward(
    self,
...
def Qwen2VLDecoderLayer_streamingllm_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
