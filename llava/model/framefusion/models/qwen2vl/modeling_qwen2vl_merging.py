...
from ...utils import TEXT_TOKEN, IGNORE_TOKEN
from ...main import find_contigious_latter_index


def replace_Qwen2VL_merging(model, sparsity=[0.1] * 28):
    ...
    if isinstance(model.model, Qwen2VLModel):
        model.model.forward = MethodType(Qwen2VLModel_merging_forward, model.model)
        
    for i, decoder_layer in enumerate(model.model.layers):
        if isinstance(decoder_layer, Qwen2VLDecoderLayer):
            decoder_layer.forward = MethodType(Qwen2VLDecoderLayer_merging_forward, decoder_layer)
            
        qwen2vl_attention_instance = decoder_layer.self_attn
        if isinstance(qwen2vl_attention_instance, Qwen2VLFlashAttention2):
            qwen2vl_attention_instance.forward = MethodType(partial(Qwen2VLFlashAttention2_merging_forward, model=model), qwen2vl_attention_instance)
        else:
            raise TypeError("language model is not Qwen2.")
     
def Qwen2VLFlashAttention2_merging_forward(
    self,
        ...
        if prune_num > 0:
            ...
            for i in range(patch_num):
                ...
                if this_patch_token_index.shape[-1] > 1:
                    ...
                    token_similarity[:, this_patch_token_index[1:]] = this_patch_similarity[:, 1:]
            ...
            token_index_by_patch = torch.cat(token_index_by_patch, dim=0).reshape(1, -1)  # shape (batch_size, q_len),
...
def Qwen2VLDecoderLayer_merging_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def Qwen2VLModel_merging_forward(
    self,
...
def Qwen2VLCausal_prepare_inputs_for_generation(
    ...
    **kwargs,
    ...
    patch_type = [TEXT_TOKEN] * image_token_start_index + list(range(patch_num)) * n_frames + [TEXT_TOKEN] * (original_length - image_token_end_index) # [1, N]
