...
from ...models.llava_video.modeling_llava_video import prepare_inputs_labels_for_multimodal_get_patch_type
...
from transformers.models.qwen2.modeling_qwen2 import Qwen2SdpaAttention, Qwen2DecoderLayer, Qwen2Model, Qwen2FlashAttention2
from functools import partial
try:
...
def replace_Qwen2_fastv(model, fastv_k = 3, fastv_r = 0.5):
    ...
    for i, decoder_layer in enumerate(model.model.layers):
        ...
        if isinstance(qwen2_attention_instance, Qwen2SdpaAttention) or isinstance(qwen2_attention_instance, Qwen2FlashAttention2):
        ...
        if use_cache:
            if hidden_states.shape[1] != 1:
                if layer_idx<FASTV_k:
                    ...
                    FASTV_image_token_length = (model.model.image_token_length - (model.original_length - hidden_states.shape[1])).item()
                    # compute pruned tokens, generate fastv sign
                    last_layer_attention = layer_outputs[1]
                    # compute average attention over different head
                    last_layer_attention_avg = torch.mean(last_layer_attention, dim=1)[0]
