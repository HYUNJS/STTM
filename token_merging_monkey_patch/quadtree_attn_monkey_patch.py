    ...
    for decoder_layer in self.layers:
        if is_prefilling and decoder_layer.self_attn.layer_idx == self.sa_start_layer_idx:
            ...
            quadtree_features, _, quadtree_tyxyx_tlbr = get_quadtree_features(video_features, self.sa_tree_thresh, self.sa_tree_temporal_thresh, self.sa_tree_root_level,
                                                        self.sa_tree_weighted_avg, slow_ver=self.sttm_slow_ver, head_dim=head_dim)
        ...
        #     curr_seq_len = past_key_values.get_seq_length(decoder_layer.self_attn.layer_idx)
        #     cache_position = torch.tensor([curr_seq_len], device=self.device, dtype=torch.long)
...
def replace_qwen2_with_quadtree_attn(sa_start_layer_idx=0, sa_tree_thresh=0.90, sa_tree_temporal_thresh=-1.0, sa_tree_root_level=0, 
                                    sa_tree_weighted_avg=False, sttm_slow_ver=False, sim_per_head=False, **kwargs):
